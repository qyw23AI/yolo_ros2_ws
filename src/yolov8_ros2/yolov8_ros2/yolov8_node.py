...
class YoloDectNode(Node):
    ...
    def dectshow(self, results, height, width):
        ...
        for result in results[0].boxes:
            ...
            # 提取边界框坐标并转换为整数
            boundingBox.xmin = np.int64(result.xyxy[0][0].item())  # 左上角x坐标
            boundingBox.ymin = np.int64(result.xyxy[0][1].item())  # 左上角y坐标
            boundingBox.xmax = np.int64(result.xyxy[0][2].item())  # 右下角x坐标
            boundingBox.ymax = np.int64(result.xyxy[0][3].item())  # 右下角y坐标
